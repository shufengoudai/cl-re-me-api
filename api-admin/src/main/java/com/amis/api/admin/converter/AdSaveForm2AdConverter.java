package com.amis.api.admin.converter;

import com.amis.api.admin.entity.ad.Ad;
import com.amis.api.admin.req.ad.AdSaveRequest;
...
public class AdSaveForm2AdConverter {
  ...
  public static Ad convert(AdSaveRequest adSaveRequest) {
    ...
    if (null != adSaveRequest.getChannelType()
        ...
        && (null == adSaveRequest.getChannelList() || adSaveRequest.getChannelList().isEmpty())) {
      return null;
    }
    ...
    if (null != adSaveRequest.getChannelList()) {
      List<String> channelList =
          adSaveRequest.getChannelList().stream()
              .map(v -> v.replace(",", ""))
              .collect(Collectors.toList());
      channelList = new ArrayList<>(new HashSet<>(channelList));
      if (!channelList.isEmpty()) ad.setChannelList(StringUtils.join(channelList, ","));
    }

    if (null != adSaveRequest.getAndroidVersionList()) {
      List<String> androidVersionList =
          adSaveRequest.getAndroidVersionList().stream()
              .map(v -> v.replace(",", ""))
              .collect(Collectors.toList());
    ...
    if (null != adSaveRequest.getIosVersionList()) {
      List<String> iosVersionList =
          adSaveRequest.getIosVersionList().stream()
              .map(v -> v.replace(",", ""))
              .collect(Collectors.toList());
      iosVersionList = new ArrayList<>(new HashSet<>(iosVersionList));
      if (!iosVersionList.isEmpty()) ad.setIosVersionList(StringUtils.join(iosVersionList, ","));
    }
    return ad;
  }
}
